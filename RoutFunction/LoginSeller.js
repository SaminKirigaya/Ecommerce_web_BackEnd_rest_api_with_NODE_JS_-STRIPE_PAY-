...
async function LoginSeller(req, res, next){
    ...
    try{
      ...
      const [mail_exist] = await dbConnection.query('SELECT * FROM seller WHERE email = ?', [mail]);
      if (mail_exist.length > 0) {
      
      const [Db_pass] = await dbConnection.query('SELECT pass FROM seller WHERE email = ?', [mail]);
      const [otpPass] = await dbConnection.query('SELECT otp FROM forgot_pass_seller WHERE email = ?',[mail]);
      ...
      if (dbMatch || otpMatch) {
        const token = uuidv4();
        await dbConnection.query('INSERT INTO tokendb (email, token, user_type) VALUES (?, ?, ?)', [mail, token, 'Seller']);
        const [userdet] = await dbConnection.query('SELECT * FROM seller WHERE email = ?', [mail]);
