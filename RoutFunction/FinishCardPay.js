...
async function FinishCardPay(req, res, next){
    try{
        ...
        if(incart.length>0){
            ...
            incart.map(async(per)=>{
                if(per.event_or_not == 'not event'){
                    ...
                    await dbConnection.query('INSERT INTO delivery_order (product_slno, user_slno, seller_slno, payment_option, estimeted_delivery, price, delivery_address, delivery_status, amount, event_or_not, uuid, name, stripeToken) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',[per.product_slno, per.user_slno, per.seller_slno, 'Payment Done', delDate, price, userData[0].delivery_address, 'On Process', per.myamount, per.event_or_not, buyno, prdData[0].name, token]);
                    uuids.push(buyno);
                }else{
                    var buyno = uuidv4();
