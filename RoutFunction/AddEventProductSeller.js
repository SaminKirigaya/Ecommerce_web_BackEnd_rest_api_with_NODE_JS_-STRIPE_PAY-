...
async function AddEventProductSeller(req, res, next){
    ...
    if (!img1mimetype || !img1extname || !img2mimetype || !img2extname || !img3mimetype || !img3extname) {
        ...
        const eventName = req.body.eventData;
        ...
        const [success] = await dbConnection.query('INSERT INTO event_products (name, type, brand, seller_slno, amount_left, price, image1, image2, image3, description, rating, total_view, like_amount, dislike_amount, event_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [intro, prdType, prdBrand, usersl, amountLeft, price, globalImage1, globalImage2, globalImage3, desc , 0, 0, 0, 0, eventName]);
        if(success){
...
module.exports = AddEventProductSeller;