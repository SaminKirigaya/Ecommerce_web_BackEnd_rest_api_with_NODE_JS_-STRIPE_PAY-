...
async function AllmyconfirmedPrd(req, res, next){
    try{

        const   {usersl} = req.params;
        const   [prdData] = await dbConnection.query('SELECT * FROM delivery_order WHERE user_slno = ?',[usersl]);
        

        if(prdData.length>0){

            await Promise.all(
                prdData.map(async(each)=>{
...
module.exports = AllmyconfirmedPrd;