...
async function AllEventPrdData(req, res, next){
    try{
        const {sln} = req.params;
        const [prdData] = await dbConnection.query('SELECT * FROM event_products WHERE slno = ?',[sln]);
        if(prdData.length>0){
...
module.exports = AllEventPrdData;