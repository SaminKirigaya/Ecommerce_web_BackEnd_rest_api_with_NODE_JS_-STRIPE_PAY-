...
async function userwhocommented(req, res, next){
    try{
        const {sln} = req.params;
        const [prdComs] = await dbConnection.query('SELECT * FROM product_comment WHERE product_slno = ?',[sln]);
        if(prdComs.length>0){
            await Promise.all(
...
module.exports = userwhocommented;