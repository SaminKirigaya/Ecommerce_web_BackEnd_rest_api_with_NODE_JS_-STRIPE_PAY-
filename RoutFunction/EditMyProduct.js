...
async function EditMyProduct(req, res, next){
    ...
    try{
        ...
        const [prdExist] = await dbConnection.query('SELECT * FROM product WHERE slno = ?',[productSln]);
        if(prdExist.length>0){
            if(prdExist[0].seller_slno == usersl){
                ...
                const [isUpdated] = await dbConnection.query('UPDATE product SET name=?, type=?, brand=?, amount_left=?, price=?, description=? WHERE slno = ?',[intro, prdType, prdBrand, amountLeft, price, desc, productSln]);
                if(isUpdated){
