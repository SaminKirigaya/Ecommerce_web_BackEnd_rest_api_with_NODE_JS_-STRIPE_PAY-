...
async function AuthenSeller(req, res, next){
    const authorizationHeader = req.headers['authorization'];
    const {usersl} = req.params;
    const token = authorizationHeader.split(' ')[1];
    const [user] = await dbConnection.query('SELECT * FROM seller WHERE slno = ?',[usersl]);
    if(user.length > 0){
        ...
        if(tokenz.length>0){
            ...
            if(user_role[0].user_type == 'Seller'){

                const [user_mail] = await dbConnection.query('SELECT email FROM seller WHERE slno = ?',[usersl]);
